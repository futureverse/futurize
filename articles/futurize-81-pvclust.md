...
**[pvclust](https://cran.r-project.org/package=pvclust)** functions,
specifically
[`pvclust()`](https://rdrr.io/pkg/pvclust/man/pvclust.html).

The **[pvclust](https://cran.r-project.org/package=pvclust)** package
provides hierarchical clustering with p-values (AU: Approximately
Unbiased p-value, BP: Bootstrap Probability) calculated via multiscale
bootstrap resampling. This method is computationally intensive because
it requires repeating the clustering process for many bootstrap
replicates at different scales. These calculations are naturally
independent and thus excellent candidates for parallelization.

### Example: Hierarchical clustering with p-values

The core function
[`pvclust()`](https://rdrr.io/pkg/pvclust/man/pvclust.html) performs
...
- [`pvclust()`](https://rdrr.io/pkg/pvclust/man/pvclust.html) with
