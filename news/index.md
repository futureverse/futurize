...
- Add support for domain-specific CRAN package **DiceKriging**,
  e.g. `m <- km(~., design = design, response = response, multistart = 8) |> futurize()`.

- Add support for domain-specific CRAN package **ez**,
  e.g. `rt <- ezBoot(data = ANT, dv = rt, wid = subnum, within = .(cue, flank), between = group) |> futurize()`.
